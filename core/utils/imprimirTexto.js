...
const { warn, error, info } = require("./printerLogger");
...
function imprimirTexto({ impressora, msg }) {
  return new Promise(async (resolve) => {
    ...
    const resultado = await enqueuePrintJob(async () => {
      ...
      info('Linux: método de impressão selecionado (utils)', {
        metadata: {
          impressora,
      ...
      for (const attempt of attempts) {
        try {
          ...
          info('Impressão Linux enviada com sucesso (utils)', {
            metadata: { impressora, metodo: attempt.devicePath || attempt.type }
          });
          success = true;
          break;
        } catch (err) {
          warn(`Tentativa ${attempt.type} falhou (utils)`, {
