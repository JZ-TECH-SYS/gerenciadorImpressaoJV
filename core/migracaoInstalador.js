...
const RELEASES_LATEST_API = 'https://api.github.com/repos/JZ-TECH-SYS/gerenciadorImpressaoJV/releases/latest';
...
function getPerUserExePath() {
    ...
    return path.join(localAppData, 'Programs', 'jv-printer', 'jv-printer.exe');
}
...
async function offerPerUserMigration({ app, dialog, toast }) {
    ...
    const escolha = await dialog.showMessageBox({
        type: 'info',
        title: 'Atualizacao do JV Printer',
    ...
    try {
        ...
        const destino = path.join(os.tmpdir(), 'jv-printer-setup-migracao.exe');
