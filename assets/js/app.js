...
function irParaAba(tab) {
  ...
  if (tab === 'impressao') carregarImpressoras();
...
function avaliar() {
  ...
  // ── IMPRESSÃO (o coração deste app) tem prioridade no semáforo ──
  const imp = o.printing || {};
  if (o.configured && !imp.impressora) {
    return {
      nivel: 'problema',
      titulo: 'Falta escolher a impressora',
      explicacao: 'Sem impressora definida, os pedidos não saem no balcão. Escolher leva 10 segundos.',
      acao: { label: 'Escolher impressora', fn: () => irParaAba('impressao') }
    };
  }
  if (imp.configurada && imp.pausadoPeloUsuario) {
    return {
      nivel: 'atencao',
      titulo: 'Impressão pausada por você',
      explicacao: 'Os pedidos estão ficando aguardando no sistema e nada será impresso até retomar.',
      acao: { label: 'Retomar impressão', fn: alternarImpressao }
    };
  }
  if (imp.ultimoErroImpressao && (Date.now() - new Date(imp.ultimoErroImpressao.em).getTime()) < 10 * 60 * 1000) {
    return {
      nivel: 'atencao',
      titulo: 'A última impressão falhou',
      explicacao: (imp.ultimoErroImpressao.erro || 'Erro na impressora') + '. Verifique papel/energia da impressora e faça um teste.',
      acao: { label: 'Testar impressão', fn: () => irParaAba('impressao') }
    };
  }
...
function renderTudo() {
  ...
  if (o) {
    ...
    const imp2 = o.printing || {};
    const impOk = imp2.rodando && !imp2.pausadoPeloUsuario;
    $('dot-imp').className = 'dot ' + (!imp2.impressora ? 'dot-bad' : (imp2.pausadoPeloUsuario ? 'dot-warn' : (impOk ? 'dot-ok' : 'dot-warn')));
    $('card-imp').textContent = !imp2.impressora ? 'Sem impressora' : (imp2.pausadoPeloUsuario ? 'Pausada por você' : (impOk ? 'Automática' : 'Parada'));
    $('card-imp-sub').textContent = imp2.impressora ? (imp2.impressora + ' · hoje: ' + (imp2.impressoesHoje ?? 0) + ' impressas') : 'escolha na aba Impressão';

    $('sw-impressao').checked = !imp2.pausadoPeloUsuario;
    $('imp-resumo').textContent = imp2.impressora
      ? ('Impressora: ' + imp2.impressora + ' · hoje: ' + (imp2.impressoesHoje ?? 0) + ' impressas' + (imp2.ultimaImpressaoEm ? (' · última: ' + new Date(imp2.ultimaImpressaoEm).toLocaleTimeString('pt-BR')) : ''))
      : 'Nenhuma impressora escolhida ainda.';
    const impAlerta = $('imp-alerta');
    const msgImp = imp2.pausadoPeloUsuario
      ? 'Impressão pausada por você — os pedidos aguardam no sistema.'
      : (imp2.ultimoErroImpressao ? ('Último erro: ' + imp2.ultimoErroImpressao.erro) : '');
    impAlerta.textContent = msgImp;
    impAlerta.classList.toggle('d-none', !msgImp);
...
/* ── ações: Impressão ── */
async function alternarImpressao() {
  await window.api.togglePrinting();
  pollOverview();
}
$('sw-impressao').addEventListener('change', alternarImpressao);

async function carregarImpressoras() {
  const sel = $('imp-select');
  try {
    const [nomes, atual] = await Promise.all([
    ...
    if (sel.options.length <= 1) {
      const opt = document.createElement('option');
      opt.textContent = 'Nenhuma impressora encontrada'; opt.disabled = true;
      sel.appendChild(opt);
    }
  } catch (e) {
...
$('btn-imp-salvar').addEventListener('click', async () => {
  ...
  if (!nome) { $('imp-feedback').textContent = 'Escolha uma impressora na lista.'; return; }
  const r = await window.api.setDefaultPrinter(nome);
  $('imp-feedback').textContent = r?.status === 'success' ? ('Impressora salva: ' + nome + '. Impressão automática ativa.') : (r?.message || 'Erro ao salvar.');
  pollOverview();
});

$('btn-imp-teste').addEventListener('click', async () => {
  const b = $('btn-imp-teste');
  b.disabled = true; b.textContent = 'Imprimindo teste…';
  try {
    const r = await window.api.testPrint();
    $('imp-feedback').textContent = r?.message || '';
  } finally {
    b.disabled = false; b.textContent = 'Teste de impressão';
  }
});
